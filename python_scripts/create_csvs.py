...
def add_random_item_sold(item_sold, order_num, prices):
  ...
  cost = float(prices[index][1:len(prices[index])])
  item_sold.append({"ITEM_ID" : len(item_sold) - 1, "MENU_ITEM_ID" : index, "ORDER_ID" : order_num, "INVENTORY_ID" : "NULL", "ITEM_SOLD_QUANTITY" : 1}, ignore_index=True)
  return cost

def add_order(orders, item_sold, date, prices):
  num_items = random.randint(1,10)
  cost = 0
  order_num = len(orders)
  for i in range(num_items):
    cost += add_random_item_sold(item_sold, order_num, prices)

  orders.append({'ORDER_ID' : len(orders) - 1, 'DATE_ORDERED' : date, 'ORDER_COST' : cost }, ignore_index=True)
  return cost
...
for i in range(365):
  ...
  for j in range(num_orders):
    total_cost += add_order(orders, item_sold, date, prices)
print(total_cost)
